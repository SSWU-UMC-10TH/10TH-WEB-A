...
const HomePage = () => {
  ...
  const { isSideOpen, open, close, toggle } = useOutletContext<HomeContextType>();
  ...
  return (
    <div className="flex min-h-screen bg-[#0f0f0f] relative overflow-x-hidden">
      <div className={`absolute z-40 h-full transition-transform duration-300 ease-in-out ${isSideOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <SideBar />
      </div>
